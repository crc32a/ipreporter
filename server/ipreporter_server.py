...
VERSION = ["ipreporterserver",1,0,0]
...
def getRecords(record_file):
    full_path = os.path.abspath(os.path.expanduser(record_file))
    if not os.path.isfile(full_path):
        open(full_path,"w").write("{}")
...
class IpReporter(object):
    def __init__(self,cred,recordfile,lfp):
        self.lfp = lfp
        self.cred = cred
    ...
    def setIp(self,name,val):
        records = getRecords(self.recordfile)
        records[name]=val
    ...
    def getAll(self):
        records = getRecords(self.recordfile)
        return [(k,v) for (k,v) in records.iteritems()]
    ...
    def delAll(self):
        ...
        out = []
        for k in keys:
            out.append((k,records[k],"DELETED"))
            del records[k]
        save_json(self.recordfile,records)
        return out        
...
if __name__ == "__main__":
    ...
    if len(sys.argv)<2:
        config_file = "/etc/ipreporter/conf.json"
    else:
    ...
    try:
        ...
        s.register_instance(IpReporter(CRED,RECORDFILE,lf),allow_dotted_names=True)
    except:
